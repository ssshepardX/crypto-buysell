...
import { Signal as AnalysisType } from '@/hooks/useSignalData';
...
const AnalysisCard: React.FC<AnalysisCardProps> = ({ name, symbol, price, change24h, isFavorite, onToggleFavorite, analysisData }) => {
  // Handle both legacy and new AI comment structures
  const aiComment = analysisData?.ai_comment as AICommentData | undefined;

  const riskScore = analysisData?.risk_score || aiComment?.risk_score || 0;
  const summary = aiComment?.summary || aiComment?.riskAnalysis;
  const likelySource = analysisData?.likely_source || aiComment?.likely_source;
  const actionableInsight = analysisData?.actionable_insight || aiComment?.actionable_insight || aiComment?.tradingAdvice;
  const volumeMultiplier = analysisData?.volume_multiplier || 1;
